...
class MainScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (isMobile) {
            this.cellSize = Math.min(100, Math.floor((width - 20) / 6));
        ...
        const layerMinRadius = [0.5, 1.0, 1.5];
        const layerMaxRadius = [1.5, 2.0, 3.0];
        ...
        for (let j = 0; j < 3; j++) {
            ...
            for (let i = 0; i < layerCounts[j]; i++) {
                ...
                const r = Phaser.Math.FloatBetween(layerMinRadius[j], layerMaxRadius[j]);
        ...
        this.scoreText = this.add.text(20, 115, T[l].score + formatNumber(0), { fontFamily: FONT_FAMILY, fontSize: fontSize, fill: '#ffd700', shadow: { offsetX: 0, offsetY: 0, color: '#ffd700', blur: 10, fill: true } });
        ...
        this.highScoreText = this.add.text(width - 20, 115, T[l].highscore + formatNumber(this.highScore), { fontFamily: FONT_FAMILY, fontSize: fontSize, fill: '#aaa', shadow: { offsetX: 0, offsetY: 0, color: '#aaa', blur: 10, fill: true } }).setOrigin(1, 0);

        // Merge count text
        this.mergeCountText = this.add.text(width / 2, 115, T[l].merges + this.mergeCount, { fontFamily: FONT_FAMILY, fontSize: fontSize, fill: '#fff', shadow: { offsetX: 0, offsetY: 0, color: '#fff', blur: 10, fill: true } }).setOrigin(0.5, 0);
    ...
    updateBackgroundGradient() {
        ...
        if (this.bgRect) {
            this.bgRect.clear();
            this.bgRect.fillGradientStyle(color1, color1, color2, color2, 1);
